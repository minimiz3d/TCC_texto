...
Neste contexto, soluções baseadas em ferramentas como redes de sensores podem ser utilizadas, onde o objetivo destas seja indicar de forma automatizada a disponibilidade de vagas em determinado local, contribuindo assim para tornar as cidades mais informatizadas, e portanto mais inteligentes. No entanto, ainda que o uso destas redes apresente diversos benefícios em inúmeras áreas, cenários compostos por WSNs (\textit{Wireless Sensor Networks}) apresentam vários complicadores em sua implementação, como a necessidade de empregar fontes de alimentação capazes de disponibilizar energia de forma segura durante longos períodos de tempo; proteção dos elementos perceptores, que podem não ser resistentes à água ou altas e baixas temperaturas, por exemplo; robustez e segurança na comunicação para que se tenha integridade dos dados; entre outros desafios que impedem um desenvolvimento mais acelerado destes sistemas \cite{puccinelli2005wireless}. 
...
Este capítulo tem como objetivo abordar brevemente diferentes soluções possíveis para o mesmo problema, isto é, aplicações que visem monitorar o estado das vagas de um estacionamento ao ar livre.
...
Dessa forma, fica claro que há inúmeras dificuldades em se empregar um sistema que seja vantajoso em muitos aspectos. A seguir serão apresentados os tipos mais comuns de sensores utilizados para este tipo de aplicação, sendo descritos pontos positivos e negativos de cada um.
